...
class IHarvesterEntity(model.Schema):
...
@implementer(IHarvesterEntity)
class HarvesterEntity(Container):
    """Harvester Field Config Content Type."""

    @property
    def fields(self):
        fields = []
        for field in CT_ENTITY_RELATION.values():
            fields += getattr(self, field, [])
        return fields


class EntityDefaultFactory(DexterityFactory):
    """Custom DX factory for Harvester Field Config."""

    def __init__(self):
        self.portal_type = c.CT_HARVESTER_ENTITY

    def __call__(self, *args, **kw):
        # Fix: get context and maybe change it
        from pkan.dcatapde.api.harvester_entity \
            import clean_entity
        data, errors = clean_entity(**kw)
