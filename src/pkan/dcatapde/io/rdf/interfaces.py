...
class IGenericObject2Surf(IObject2Surf):
    ...
    portalType = Attribute(
        u'The portal type of the context, will be used as resource class',
    )
...
class ISurfResourceModifier(Interface):
    ...
    def run(resource):
        """Get the rdf resource as argument.

        Allow it to be changed in place.
        """
...
class IField2Surf(Interface):
    ...
    def value(context):
        """Return the value in format understandable by SURF."""
...
class IValue2Surf(Interface):
    ...
    def __call__(*args, **kwds):
        """Return a value for surf.

        Suitable to be assigned as a value to an rdf resource attribute.
        """
